...
def extract_gov24_detail_row(detail_payload: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(detail_payload, dict):
        return {}
    data_node = detail_payload.get("data")
    if isinstance(data_node, list) and data_node:
        return data_node[0] if isinstance(data_node[0], dict) else {}
...
def normalize_gov24(db: Session) -> int:
    ...
    for source_policy_id, list_raw in list_map.items():
        detail_raw = extract_gov24_detail_row(detail_map.get(source_policy_id, {}))
        condition_raw = condition_map.get(source_policy_id, {})
        policy_id = make_policy_id("gov24", source_policy_id)
        now = datetime.utcnow()

        title = clean_text(value_of(detail_raw, "서비스명")) or clean_text(value_of(list_raw, "서비스명")) or source_policy_id
        ...
        for law_name in normalize_split_items(clean_text(value_of(detail_raw, "법령"))):
            db.add(PolicyLaw(policy_id=policy_id, law_name=law_name[:500], law_type="law", source="gov24"))
