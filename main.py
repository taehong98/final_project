...
async def post_analyze(req: AnalyzeRequest):
    ...
    user = {
        ...
        "income_percent": req.income_percent,
        "education_level": req.education_level or "",
        "language": req.language or "ko",

        # scoring.py / search 쪽 한글 키
        "나이": req.age,
        "연소득": annual_income,
        ...
        "household_size": req.household_size or 1,
        "region": req.region,
        "employment_status": req.employment_status,
        "disability": req.disability or "없음",
        "veteran": req.veteran or False,
        "multicultural": req.multicultural or False,
    }
    ...
    # analysis.py가 만든 다국어 대시보드 데이터를 우선 사용
    _gpt_stats       = gpt_result.get("대시보드통계", {}) or {}
    _gpt_dashboard   = gpt_result.get("dashboard_data", {}) or {}
    _gpt_user        = _gpt_dashboard.get("user_profile", {}) or {}
    _gpt_condition   = (_gpt_dashboard.get("condition_form", {}) or {}).get("current_values", {}) or {}
    _gpt_dash_stats  = _gpt_dashboard.get("dashboard_stats", {}) or {}
    _gpt_stats_block = _gpt_dashboard.get("stats", {}) or {}
    _gpt_preview     = _gpt_dashboard.get("portfolio_preview", {}) or {}
    ...
    dashboard_data = {
        "user_profile": {
            "user_name":        req.user_name,
            "updated_at_label": _gpt_user.get("updated_at_label", now_label),
            "region_label":     _gpt_user.get("region_label", req.region),
            "total_score":      avg_pct,
            "score_max":        100,
            "tags":             _gpt_user.get("tags", fallback_tags),
        },
        "condition_form": {
            "current_values": _gpt_condition or fallback_current_values
        },
