...
type SupportedLang = "ko" | "en" | "zh" | "ja" | "vi";
...
const languageOptions: Array<{ code: SupportedLang; label: string }> = [
  { code: "ko", label: "한국어" },
...
export function Dashboard() {
  ...
  const [language, setLanguage] = useState<SupportedLang>("ko");
  ...
  const loadPolicyBundle = async (policyId: string, nextTab: ScreenTab, langCode: SupportedLang = language) => {
    const [detailData, prepData] = await Promise.all([
      requestJson<PolicyDetail>(`/policies/${policyId}/detail?lang=${langCode}`),
  ...
  const handleLanguageChange = async (nextLanguage: SupportedLang) => {
    ...
    setLoading(true);
    try {
      await loadPolicyBundle(selectedPolicyId, activeTab, nextLanguage);
      const languageLabel = languageOptions.find((option) => option.code === nextLanguage)?.label ?? nextLanguage;
      setMessage(`${languageLabel} 기준으로 상세 분석을 다시 불러왔습니다.`);
    } catch {
      setMessage("언어 변경 중 상세 정보를 다시 불러오지 못했습니다.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const result = await requestJson<SearchResponse>(`/policies/search?q=${encodeURIComponent(keyword)}&size=20&lang=${language}`);
        ...
        <div className="nav-right">
          <div className="lang-selector">
            <select
              className="lang-btn"
              value={language}
              aria-label="상세 분석 언어 선택"
              onChange={(e) => void handleLanguageChange(e.target.value as SupportedLang)}
            >
